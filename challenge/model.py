...
class DelayModel:
    ...
    def predict(
        ...
        if self._model is None:
            ...
            raise ValueError(
                "Model has not been trained. Call 'fit' method first.")
    ...
    def importer(
        ...
        Raises:
            ValueError: If the model is not trained (call 'fit' method first).

        This method serializes and saves the trained logistic regression model to
        'trained_log_reg-0.1.0.pkl'. Ensure the model is trained before using this.
        """
        if self._model is None:
            # the probles is with the test
            raise ValueError(
